...
namespace lpzrobots {
  ...
  int Skeleton::getMotorNumberIntern(){
    if(conf.onlyPrimaryFunctions)
      return hipservos.size() + kneeservos.size() + ankleservos.size() + armservos.size()+ 3/*pelvis*/ ;
    else
  ...
  */
  void Skeleton::setMotorsIntern(const double* motors, int motornumber){
    ...
    cout << motornumber << " " << getMotorNumberIntern() << " " << n << endl;
  ...
  int Skeleton::getSensorNumberIntern(){
    ...
    numberSensors += 3; // three position sensors
